...
for i in 12500.5 25000 37500 50000 75000
do
psql -e -v x=$i assign2 <<EOF
	...
	\timing on
	SELECT COUNT(*) FROM R, S WHERE R.A = S.A AND R.B = S.A AND R.B <= :x;
	\timing off
	-- examine query plan
	EXPLAIN ANALYZE SELECT COUNT(*) FROM R, S WHERE R.A = S.A AND R.B = S.A AND R.B <= :x;
	\q
EOF
done
