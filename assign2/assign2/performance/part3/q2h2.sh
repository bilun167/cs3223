...
psql -e -v x=$i assign2 <<EOF
	...
	\timing on
	SELECT COUNT(*) FROM S WHERE EXISTS (SELECT * FROM R WHERE R.A = S.A AND R.B <= :x);
	\timing off
	-- examine query plan
	EXPLAIN ANALYZE SELECT COUNT(*) FROM S WHERE EXISTS (SELECT * FROM R WHERE R.A = S.A AND R.B <= :x);
	\q
EOF
done
